...
function testStaticLoadingScreen() {
  ...
  filesToCheck.forEach((file) => {
    try {
      ...
      // 检查是否包含静态加载屏幕
      const hasLoadingDiv = content.includes(
        '<div id="loading" class="loading-screen">',
      );
      const hasLoadingCSS =
        content.includes(".loading-screen {") ||
        content.includes(".loading-screen{");
      const isInBody =
        content.indexOf('<div id="loading"') > content.indexOf("<body>");
      const isBeforeMain =
        content.indexOf('<div id="loading"') < content.indexOf("<main>");
...
function testComponentManagerFix() {
  ...
  try {
    const componentManagerPath = path.join(
      __dirname,
    ...
    const hasExistingCheck =
      content.includes("existingLoading") ||
      content.includes("getElementById('loading')");
    const hasBackupComment =
      content.includes("备用") || content.includes("backup");
    const hasConsoleLog =
      content.includes("使用HTML中的静态加载屏幕") ||
      content.includes("HTML中无加载屏幕");
...
function testLoadingHideLogic() {
  ...
  const filesToCheck = [
    ...
    {
      path: "assets/js/modules/components/component-manager.js",
      name: "component-manager.js",
    },
  ...
  filesToCheck.forEach((file) => {
    try {
      ...
      // 检查是否有重复检查
      const hasHiddenCheck =
        content.includes("classList.contains('hidden')") ||
        content.includes("style.display === 'none'");
      const hasDelayedHide =
        content.includes("setTimeout") && content.includes("hidden");
      const hasUnifiedHide =
        content.includes("componentManager.hideLoadingScreen") ||
        content.includes("hideLoadingScreen");
    ...
    try {
      ...
      if (orderCorrect && foundScripts >= 3) {
        addTestResult(
          `脚本顺序-${file.name}`,
          "pass",
          `脚本加载顺序正确 (${foundScripts}/${scriptOrder.length})`,
        );
      } else if (foundScripts >= 3) {
        addTestResult(
          `脚本顺序-${file.name}`,
          "warn",
          `脚本数量足够但顺序可能有问题 (${foundScripts}/${scriptOrder.length})`,
        );
      } else {
        addTestResult(
          `脚本顺序-${file.name}`,
          "fail",
          `关键脚本缺失 (${foundScripts}/${scriptOrder.length})`,
        );
      }
    } catch (error) {
...
function generateTestReport() {
  ...
  const successRate = (
    (testResults.passed / testResults.details.length) *
    100
  ).toFixed(1);
