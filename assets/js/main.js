...
function performSearch() {
    ...
    if (!productsData || products.length === 0) {
        if (searchResults) {
            searchResults.innerHTML = '<div class="search-result-item">数据加载中...</div>';
    ...
    products.forEach(product => {
        ...
        const oeMatch = product.oe_number && product.oe_number.toLowerCase().includes(query);
        const compatibilityMatch = product.compatibility && product.compatibility.toLowerCase().includes(query);
        const notesMatch = product.notes && product.notes.toLowerCase().includes(query);
        const featuresMatch = product.features && product.features.toLowerCase().includes(query);

        if (nameMatch || brandMatch || modelMatch || descMatch || skuMatch || oeMatch || compatibilityMatch || notesMatch || featuresMatch) {
            // 使用翻译系统获取分类名称
            const getCategoryDisplayName = (category) => {
                ...
                return categoryMap[category] || getTranslation('categories.others') || '产品';
            };
...
function displaySearchResults(results) {
    ...
    if (results.length === 0) {
        searchResults.innerHTML = '<div class="search-result-item">未找到相关产品</div>';
    } else {
...
async function loadProductShowcase() {
    ...
    try {
        ...
        const sortedProducts = window.ProductSortUtils.getHomePageProducts(products, 9);
...
function inquireProduct(productName) {
    // 从公司信息中获取WhatsApp号码
    const whatsappNumber = window.companyInfo?.contact?.whatsapp?.replace(/\D/g, '') || '8613656157230';
    // WhatsApp询价功能
...
function handleFormSubmit(event) {
    ...
    .then(data => {
        ...
        if (data.success) {
            showNotification('📧 询价信息发送成功！我们将在24小时内回复您。', 'success');
            form.reset();
        } else {
...
function showAlternativeContact() {
    const whatsappNumber = window.companyInfo?.contact?.whatsapp?.replace(/\D/g, '') || '8613656157230';
    const phone = window.companyInfo?.contact?.phone?.replace(/\D/g, '') || '8613656157230';
    const email = window.companyInfo?.contact?.email || 'sales03@diamond-auto.com';
