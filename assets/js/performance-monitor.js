...
(function () {
  ...
  const PerformanceMonitor = {
    ...
    startMonitoring: function () {
      ...
      if (document.readyState === "loading") {
        document.addEventListener("DOMContentLoaded", () => {
          ...
          console.log(
            `📊 DOM加载完成: ${(this.metrics.domContentLoaded - this.metrics.pageLoadStart).toFixed(2)}ms`,
          );
        });
      } else {
        this.metrics.domContentLoaded = performance.now();
      ...
      if (document.readyState !== "complete") {
        window.addEventListener("load", () => {
          ...
          console.log(
            `📊 窗口加载完成: ${(this.metrics.windowLoaded - this.metrics.pageLoadStart).toFixed(2)}ms`,
          );
      ...
      const report = {
        ...
        初始化分析: {
          ...
          重复初始化统计: Object.fromEntries(
            Array.from(this.initCounters.entries()).filter(
              ([key, count]) => count > 1,
            ),
          ),
        },
    ...
    calculateAverageLoadTime: function () {
      ...
      const total = this.metrics.resourceLoadTimes.reduce(
...
window.checkDoubleRefresh = function () {
  if (window.PerformanceMonitor) {
    ...
    const repeatedInits = Array.from(
      window.PerformanceMonitor.initCounters.entries(),
