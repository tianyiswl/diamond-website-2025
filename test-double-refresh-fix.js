...
function testPageLoadManager() {
  ...
  try {
    const pageLoadManagerPath = path.join(
      __dirname,
...
function testNavigationOptimizer() {
  ...
  try {
    const navOptimizerPath = path.join(
      __dirname,
      "assets/js/navigation-optimizer.js",
    );
    ...
    if (foundFeatures === features.length) {
      ...
      addTestResult(
        "导航优化器",
        "warn",
        `${foundFeatures}/${features.length} 功能可用`,
      );
    }
  } catch (error) {
  ...
  try {
    ...
    filesToCheck.forEach((filePath) => {
      ...
      if (fs.existsSync(fullPath)) {
        ...
        const matches = content.match(
          /addEventListener\s*\(\s*['"]DOMContentLoaded['"]|document\.addEventListener\s*\(\s*['"]DOMContentLoaded['"]/g,
        );
        ...
        if (count === 0) {
          addTestResult(
            `DOMContentLoaded-${path.basename(filePath)}`,
            "pass",
            "无重复监听器",
          );
        } else if (count === 1) {
    ...
    if (totalDOMListeners <= 2) {
      addTestResult(
        "重复事件监听器",
        "pass",
        `总计${totalDOMListeners}个DOMContentLoaded监听器，数量合理`,
      );
    } else {
      addTestResult(
        "重复事件监听器",
        "warn",
        `总计${totalDOMListeners}个DOMContentLoaded监听器，可能存在重复`,
      );
    }
  } catch (error) {
