...
class InquiryService extends BaseService {
  ...
  async createInquiry(inquiryData) {
    try {
      ...
      if (spamCheck.isSpam) {
        console.log('🚫 垃圾信息检测:', spamCheck.reasons);
        return this.error([`询价内容可能包含垃圾信息: ${spamCheck.reasons.join(', ')}`], '内容验证失败');
      }
  ...
  async batchOperation(action, inquiryIds, data = {}) {
    try {
      ...
      const result = await super.batchOperation(inquiryIds, operation);
  ...
  checkSpam(inquiryData) {
    // 更精确的垃圾关键词检测 - 避免误判正常业务词汇
    const spamKeywords = ['赌博', '博彩', '彩票', '贷款', '借钱', '色情', '成人', '约炮', '一夜情', '代开发票', '假证', '办证', '刷单', '微商招代理', '加盟代理', '投资理财', '股票', '期货', '外汇', '比特币'];

    const content = `${inquiryData.name} ${inquiryData.email} ${inquiryData.message}`.toLowerCase();

    // 检查是否包含垃圾关键词，但排除测试相关内容
    const isTestContent = content.includes('测试') || content.includes('test');
    const hasSpamKeywords = !isTestContent && spamKeywords.some(keyword =>
